...
export function setupAuth(app: Express) {
  ...
  app.get("/api/user", async (req, res) => {
    ...
    try {
      ...
      // MODIFICAÇÃO: Agora o referred_by armazena o código de referral do referenciador
      ...
      // Precisamos usar o código de referral de cada usuário nível 1
      const level1ReferralCodes = level1Referrals.map(user => user.referralCode);
      const level2Referrals = allUsers.filter(user => user.referredBy && level1ReferralCodes.includes(user.referredBy));
      
      // Contar referidos nível 3
      // Precisamos usar o código de referral de cada usuário nível 2
      const level2ReferralCodes = level2Referrals.map(user => user.referralCode);
      const level3Referrals = allUsers.filter(user => user.referredBy && level2ReferralCodes.includes(user.referredBy));
