      ...
      await db.execute(sql`
        ...
        WITH RECURSIVE referral_tree AS (
          ...
          JOIN referral_tree rt ON 
            u.referred_by::TEXT = rt.phone_number OR 
            u.referred_by::TEXT = rt.referral_code OR
            u.referred_by = rt.id
        ...
        LEFT JOIN referral_tree r ON 
          r.referred_by::TEXT = u.phone_number OR 
          r.referred_by::TEXT = u.referral_code OR
          r.referred_by = u.id
        GROUP BY u.id;
      `);
      ...
      try {
        await db.execute(sql`
          ...
          SELECT 
            u.id AS user_id,
            COUNT(CASE WHEN 
              -- Abordagem mais simples: contar apenas referidos pelo telefone ou código
              r.referred_by = u.phone_number OR r.referred_by = u.referral_code
              THEN 1 ELSE NULL END) AS level1_count,
            0 AS level2_count,
            0 AS level3_count,
            SUM(CASE WHEN 
              (r.referred_by = u.phone_number OR r.referred_by = u.referral_code) 
              AND r.has_product = true THEN 1 ELSE 0 END) AS level1_active,
            0 AS level2_active,
            0 AS level3_active
          FROM users u
          -- Usar LEFT JOIN para incluir usuários sem referidos
          LEFT JOIN users r ON 
            r.referred_by = u.phone_number OR 
